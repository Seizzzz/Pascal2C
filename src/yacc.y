...
programstruct: 	program_head ';' program_body '.'{ //正常
			   	...
			   	}|program_head ';' error '.'{ //ERROR program_body识别失败 
			   		ParseTreeHead=$$=new Type;
			   		$$->token = "programstruct";
					yyerror("fatal error in program body");
			   	}|error program_head ';' program_body '.'{ //ERROR program_head前包含非法字符 
					ParseTreeHead=$$=new Type;
					$$->token = "programstruct";
					yyerror("invalid symbol before program head", @$.first_line, @$.first_column, @2.first_line, @2.first_column-1);
				}|error program_head error program_body '.'{ //ERROR program_head前包含非法记号、缺失分号
					ParseTreeHead=$$=new Type;
					$$->token = "programstruct";
				...
				}|error program_head ';' program_body error{ //ERROR program_head前包含非法记号、缺失点号
					ParseTreeHead=$$=new Type;
					$$->token = "programstruct";
					yyerror("invalid token before program head, maybe missing keyword \"program\"", @$.first_line, @$.first_column, @2.first_line, @2.first_column-1);
					yyerror("missing a dot here", @4.last_line, @4.last_column+1);
				}|error program_head ';' error '.'{ //ERROR program_head前包含非法记号、program_body识别失败
					ParseTreeHead=$$=new Type;
					$$->token = "programstruct";
					yyerror("invalid token before program head, maybe missing keyword \"program\"", @$.first_line, @$.first_column, @2.first_line, @2.first_column-1);
...
program_head: 	PROGRAM IDENTIFIER '(' idlist ')'{ //正常
				...
				}|PROGRAM error '(' idlist ')'{ //ERROR 缺少主程序名 
					$$=new Type;
					$$->token = "program_head";
...
const_declaration: 	const_declaration ';' IDENTIFIER '=' const_value{ //正常
						...
						$$->children.push_back($1); $$->children.push_back($2);
						$$->children.push_back($3); $$->children.push_back($4); $$->children.push_back($5);
					}|const_declaration ';' IDENTIFIER '=' error{ //常数初始化右值缺失 
						$$=new Type;
						$$->token = "const_declaration";
...
var_declaration: 	var_declaration ';' idlist ':' type { //正常
						...
						$$->token ="var_declaration";
						$$->children.push_back($1);$$->children.push_back($2); $$->children.push_back($3);
					}|var_declaration error idlist ':' type { //ERROR 缺少分号 
						$$=new Type;
						$$->token = "var_declaration";
						yyerror("missing a semicolon here", @1.last_line, @1.last_column+1);
					}|var_declaration ';' idlist error type { //ERROR 缺少冒号 
						$$=new Type;
						$$->token = "var_declaration";
						yyerror("missing a colon here", @3.last_line, @3.last_column+1);
					}|var_declaration ';' idlist ':' error { //ERROR type识别失败 
						$$=new Type;
						$$->token = "var_declaration";
...
type: 	TYPE{ //正常
			...
			$$->token = "type";
			yyerror("incomplete array type", &@$);
		}|ARRAY '[' period error{ //ERROR 不完整的数组类型 
			$$=new Type;
			$$->token = "type";
			yyerror("incomplete array type", &@$);
		};
...
period: period ',' UINUM RANGEDOT UINUM{ //正常
			...
			$$->children.push_back($1);$$->children.push_back($2);
			$$->children.push_back($3);$$->children.push_back($4);$$->children.push_back($5);
		}|period error UINUM RANGEDOT UINUM{ //ERROR 缺少逗号 
			$$=new Type;
			$$->token="period";
			yyerror("missing a comma here", @1.last_line, @1.last_column+1);
		}|period ',' UINUM error UINUM{ //ERROR 缺少双点号 
			$$=new Type;
			$$->token="period";
...
subprogram_declarations: 	subprogram_declarations subprogram ';'{ //正常
								...
								$$->children.push_back($1);$$->children.push_back($2);$$->children.push_back($3);
							}|subprogram_declarations subprogram error{ //ERROR 缺少分号
								$$=new Type;
								$$->token="subprogram_declarations";
								yyerror("missing a semicolon here", @2.last_line, @2.last_column+1);
							}|{ //正常
...
subprogram: subprogram_head ';' subprogram_body{ //正常
				...
				$$->token="subprogram";
				$$->children.push_back($1);$$->children.push_back($2);$$->children.push_back($3);
			}|subprogram_head error subprogram_body{ //ERROR 缺少分号 
				$$=new Type;
				$$->token="subprogram";
				yyerror("missing a semicolon here", @1.last_line, @1.last_column+1);
			};
...
subprogram_head: 	PROCEDURE IDENTIFIER formal_parameter{ //正常
						...
						$$->children.push_back($1);$$->children.push_back($2);
						$$->children.push_back($3);$$->children.push_back($4);$$->children.push_back($5);
					}|FUNCTION error formal_parameter ':' TYPE{ //ERROR 函数名缺失 
						$$=new Type;
						$$->token="subprogram_head";
						yyerror("missing function name", @1.last_line, @1.last_column+1);
					}|FUNCTION IDENTIFIER formal_parameter error TYPE{ //ERROR 缺少冒号 
						$$=new Type;
						$$->token="subprogram_head";
						yyerror("missing a colon here", @3.last_line, @3.last_column);
					}|FUNCTION IDENTIFIER formal_parameter ':' error{ //ERROR 缺少基本类型关键字
						$$=new Type;
						$$->token="subprogram_head";
					...
					}|FUNCTION error{ //ERROR 不完整的函数头 
						$$=new Type;
						$$->token="subprogram_head";
						yyerror("incomplete function head", &@$);
					}|PROCEDURE error{ //ERROR 不完整的过程头
						$$=new Type;
						$$->token="subprogram_head";
...
parameter_list: parameter_list ';' parameter{ //正常
					...
					$$->children.push_back($1);$$->children.push_back($2);$$->children.push_back($3);
				}|parameter_list error parameter{ //ERROR 缺少分号 
					$$=new Type;
					$$->token="parameter_list";
					yyerror("missing a semicolon here", @1.last_line, @1.last_column+1);
				}|parameter{ //正常
...
compound_statement: _BEGIN statement_list END{ //正常
						...
						$$->children.push_back($1);$$->children.push_back($2);$$->children.push_back($3);
					}|_BEGIN statement_list error{ //ERROR 缺少END关键字 
						$$=new Type;
						$$->token="compound_statement";
...
statement_list: statement_list ';' statement{ //正常
					...
					$$->children.push_back($1);$$->children.push_back($2); $$->children.push_back($3);
				}|statement_list error statement{ //ERROR 缺失分号 这里引发了3个规约规约冲突 
					$$=new Type;
					$$->token="statement_list";
					yyerror("missing a semicolon here", @1.last_line, @1.last_column+1);
				}|statement{ //正常
...
statement: 	variable ASSIGNOP expression{ //正常
				...
				$$->children.push_back($1);$$->children.push_back($2);
				$$->children.push_back($3);$$->children.push_back($4);$$->children.push_back($5);
			}|IF expression error statement else_part{ //ERROR 缺少then关键字 
				$$=new Type;
				$$->token="statement";
			...
			}|FOR IDENTIFIER error expression TO expression DO statement{ //ERROR 缺少赋值号
				$$=new Type;
				$$->token="statement";
				yyerror("missing assignop \":=\"", @2.last_line, @2.last_column+1);
			}|FOR IDENTIFIER ASSIGNOP expression error expression DO statement{ //ERROR 缺少关键字to 
				$$=new Type;
				$$->token="statement";
				yyerror("missing keywrod \"to\"", @4.last_line, @4.last_column+1);
			}|FOR IDENTIFIER ASSIGNOP expression TO expression error statement{ //ERROR 缺少关键字do 
				$$=new Type;
				$$->token="statement";
				...
				$$->children.push_back($1);$$->children.push_back($2);
				$$->children.push_back($3);$$->children.push_back($4);
			}|WHILE expression error statement{ //ERROR 缺少关键字do 
				$$=new Type;
				$$->token="statement";
				...
				$$->children.push_back($1);$$->children.push_back($2);
				$$->children.push_back($3);$$->children.push_back($4);
			}|REPEAT statement error expression{ //ERROR 缺少关键字until 
				$$=new Type;
				$$->token="statement";
...
id_varpart: '[' expression_list ']'{ //正常
				...
				$$->token="id_varpart";
				$$->children.push_back($1);$$->children.push_back($2);$$->children.push_back($3);
			}|'[' error{ //ERROR 不完整的数组下标列表 
				$$=new Type;
				$$->token="id_varpart";
				yyerror("incomplete expression list of array subindex", &@$);
			}|'[' expression_list error{ //ERROR 缺失右中括号 
				$$=new Type;
				$$->token="id_varpart";
...
procedure_call: IDENTIFIER{ //正常
				...
				$$->children.push_back($1);$$->children.push_back($2);
				$$->children.push_back($3);$$->children.push_back($4);
			}|IDENTIFIER '(' expression_list error{ //ERROR 缺少右括号 
				$$=new Type;
				$$->token="procedure_call";
...
expression_list: 	expression_list ',' expression{ //正常
						...
						$$->token="expression_list";
						$$->children.push_back($1);
					}|expression_list error expression{ //ERROR 缺少逗号 这里引发了一个移进规约冲突 
						$$=new Type;
						$$->token="expression_list";
						yyerror("missing a comma here", @1.last_line, @1.last_column+1);
					};
