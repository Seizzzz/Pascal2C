...
void getIdList(Type* now, vector<pair<string, int>>& res, bool reverseFlag) {
	...
	if (now->children[0]->str.empty()) {
		res.emplace_back(make_pair(now->children[2]->str, now->children[2]->lineNumber));
		getIdList(now->children[0], res, reverseFlag);
	}
	else {
		res.emplace_back(make_pair(now->children[0]->str, now->children[0]->lineNumber));
		if (reverseFlag)
...
void getArrayRangeList(Type* now, vector<pair<int, int>>& _arrayRangeList) {
	...
	_arrayRangeList.emplace_back(make_pair(str2int(now->children[loc]->str), str2int(now->children[loc + 2]->str)));
	if (loc == 2)
